...
import dao.board.BulletinBoardDAO;
...
import service.board.BulletinBoardService;
import service.board.BulletinBoardServiceImpl;
...
public class BulletinBoardController extends HttpServlet {
	...
	private static final Logger logger = LogManager.getLogger(BulletinBoardController.class); 
	private BulletinBoardService boardService;
	
	
	public BulletinBoardController() {
        super();
        boardService = new BulletinBoardServiceImpl(); 
    }
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		  logger.info("BulletinBoardController doGet"); 
	      String path = request.getRequestURI();
	      logger.info("BulletinBoardController doGet path" + path); 
	      
	      if ("/bulletinboard/bulletinView.do".equals(path)) {
	    	  ...
	    	  BulletinBoard board = boardService.getBoardById(boardId);
	    	  List<BulletinComment> sortedComments = board.getComments().stream()
	    		        .sorted(Comparator.comparing(BulletinComment::getCreateDt))
	          ...
	          request.getRequestDispatcher("/WEB-INF/jsp/bulletinBoard/bulletinView.jsp").forward(request, response);
	      } else if("/bulletinboard/bulletinCreate.do".equals(path)) {
	    	
	    	  request.getRequestDispatcher("/WEB-INF/jsp/bulletinBoard/bulletinCreate.jsp").forward(request, response);
	      } else if ("/bulletinboard/bulletinUpdate.do".equals(path)) {
	    	  
	    	  String boardId =request.getParameter("id");
	    	  BulletinBoard board = boardService.getBoardById(boardId);
	    	  request.setAttribute("board", board);
	    	  
	    	  request.getRequestDispatcher("/WEB-INF/jsp/bulletinBoard/bulletinUpdate.jsp").forward(request, response);
	      }else if("/bulletinboard/bulletinDelete.do".equals(path)) {
		      String boardId =request.getParameter("id");
		      BulletinBoard board = boardService.getBoardById(boardId);
	    	  request.setAttribute("board", board);
    	  
	    	  request.getRequestDispatcher("/WEB-INF/jsp/bulletinBoard/bulletinDelete.jsp").forward(request, response);
	      } else if ("/bulletinboard/bulletinList.do".equals(path)) {
	    	  ...
	    	  List<BulletinBoard> boardList = boardService.getBoardList(board);
	    	  ...
	    	  request.getRequestDispatcher("/WEB-INF/jsp/bulletinBoard/bulletinList.jsp").forward(request, response);
	      }
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            if ("/bulletinboard/bulletinCreate.do".equals(path)) { 
            ...
            }else if("/bulletinboard/comment/create.do".equals(path)) {
            ...
            }else if("/bulletinboard/comment/update.do".equals(path)) {
            ...
            }else if("/bulletinboard/comment/delete.do".equals(path)) {
            	int commentId =Integer.parseInt(request.getParameter("commentId"));
            	String updateId =request.getParameter("updateId");
