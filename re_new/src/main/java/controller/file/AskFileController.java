...
import service.file.AskFileService;
import service.file.AskFileServiceImpl;


@WebServlet("/askinfile/*")
@MultipartConfig(fileSizeThreshold = 1024 * 1024, // 1MB
		maxFileSize = 10 * 1024 * 1024, // 10MB
		maxRequestSize = 50 * 1024 * 1024 // 50MB
)
public class AskFileController extends HttpServlet {

	/**
	 * 
	 */
	private static final long serialVersionUID = -5939703672566257803L;
	private static final Logger logger = LogManager.getLogger(AskFileController.class);
	private AskFileService fileService;

	public AskFileController() {
		super();
		fileService = new AskFileServiceImpl();
	}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if ("/askfile/down.do".equals(path)) {
			int fileId = Integer.parseInt(request.getParameter("fileId"));
			AskPostFile file = new AskPostFile();
			file.setFileId(fileId);

			AskPostFile selectFile = fileService.getFileByFileId(file);
			...
			try {
				...
				response.getWriter().write("파일 다운로드중 오류가 발생했습니다.");
			}
		} else if ("/askfile/imgDown.do".equals(path)) {
			int fileId = Integer.parseInt(request.getParameter("fileId"));
			AskPostFile file = new AskPostFile();
			file.setFileId(fileId);
			...
			// 파일 다운로드 헤더 설정
			String downloadHeader = "inline; filename=" + URLEncoder.encode(fileName, "UTF-8");
			response.setHeader("Content-Disposition", downloadHeader);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if ("/file/imgUpload.do".equals(path)) {
				...
				jsonResponse.put("message", isUploadFile ? "파일 업로드를 성공하였습니다." : "파일 업로드를 실패하였습니다.");
				if (isUploadFile) {
