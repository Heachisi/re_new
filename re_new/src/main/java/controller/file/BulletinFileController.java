...
public class BulletinFileController extends HttpServlet {

	/**
	 * 
	 */
	private static final long serialVersionUID = -5939703672566257803L;
	private static final Logger logger = LogManager.getLogger(BulletinFileController.class);
	private BulletinFileService fileService;

	public BulletinFileController() {
		super();
		fileService = new BulletinFileServiceImpl();
	}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if ("/bulletinfile/down.do".equals(path)) {
			int fileId = Integer.parseInt(request.getParameter("fileId"));
			BulletinPostFile file = new BulletinPostFile();
			file.setFileId(fileId);

			BulletinPostFile selectFile = fileService.getFileByFileId(file);
		...
		} else if ("/bulletinfile/imgDown.do".equals(path)) {
			int fileId = Integer.parseInt(request.getParameter("fileId"));
			BulletinPostFile file = new BulletinPostFile();
			file.setFileId(fileId);
