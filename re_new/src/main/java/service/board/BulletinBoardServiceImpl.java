...
public class BulletinBoardServiceImpl implements BulletinBoardService {
	private static final Logger logger = LogManager.getLogger(BulletinBoardServiceImpl.class);
	private BulletinBoardDAO boardDAO;// DB접속용
	private BulletinFileDAO fileDAO;

	private SqlSessionFactory sqlSessionFactory; // MyBatis SQL 세션 팩토리
	private Object BulletinBoardMapper;

	/**
	 * BoardServiceImpl 생성자 //서비스 생성 이유: 비즈니스 로직을 담기 위해(하나의 비즈니스 로직은 하나의 함수만 담음)
	 */
	public BulletinBoardServiceImpl() {
		this.boardDAO = new BulletinBoardDAO();			
	...
	public BulletinBoard getBoardById(String boardId) {
		...
		BulletinBoard selectBoard = boardDAO.getBoardById(session, boardId);
		//파일 목록 조회
	...
	public boolean createBoard(BulletinBoard board, HttpServletRequest request) {
		...
		try {
			...
			List<BulletinPostFile> fileList = BulletinFileUploadUtil.uploadFiles(fileParts, "board",
	...
	public boolean updateBoard(BulletinBoard board, HttpServletRequest request) {
		...
		try {
			...
			if (result) {
				...
				// 기존 파일 조회
				List<BulletinPostFile> existingFiles = fileDAO.getFilesByBoardId(session, (board.getBoardId()));

				// 기존 파일 리스트가 있을 때
				if (existingFiles != null && existingFiles.size() > 0) {
					boolean fileExists = false;
					for (BulletinPostFile existingFile : existingFiles) {
						fileExists = false;
	...
	public List<BulletinBoard> getBoardList(BulletinBoard board) {
		...
		List<BulletinBoard> list = boardDAO.getBoardList(session, board);
