...
public class FindBoardServiceImpl implements FindBoardService {
	private static final Logger logger = LogManager.getLogger(FindBoardServiceImpl.class);
	private FindBoardDAO boardDAO;// DB접속용
	private FindFileDAO fileDAO;

	private SqlSessionFactory sqlSessionFactory; // MyBatis SQL 세션 팩토리
	private Object FindBoardMapper;

	/**
	 * BoardServiceImpl 생성자 //서비스 생성 이유: 비즈니스 로직을 담기 위해(하나의 비즈니스 로직은 하나의 함수만 담음)
	 */
	public FindBoardServiceImpl() {
		this.boardDAO = new FindBoardDAO();			
	...
	public FindBoard getBoardById(String boardId) {
		...
		FindBoard selectBoard = boardDAO.getBoardById(session, boardId);
		//파일 목록 조회
	...
	public boolean createBoard(FindBoard board, HttpServletRequest request) {
		...
		try {
			...
			List<FindPostFile> fileList = FindFileUploadUtil.uploadFiles(fileParts, "board",
	...
	public boolean updateBoard(FindBoard board, HttpServletRequest request) {
		...
		try {
			...
			if (result) {
				...
				// 기존 파일 조회
				List<FindPostFile> existingFiles = fileDAO.getFilesByBoardId(session, (board.getBoardId()));

				// 기존 파일 리스트가 있을 때
				if (existingFiles != null && existingFiles.size() > 0) {
					boolean fileExists = false;
					for (FindPostFile existingFile : existingFiles) {
						fileExists = false;
	...
	public List<FindBoard> getBoardList(FindBoard board) {
		...
		List<FindBoard> list = boardDAO.getBoardList(session, board);
	...
	public List<Product> getMyProductList(String userId) {
		SqlSession session = sqlSessionFactory.openSession();
		return boardDAO.getMyProductList(session, userId);
	}
}