...
<link rel="stylesheet" href="/css/shareview.css?ver=1.1">
			...
			<div id="editForm_${comment.commentId}" style="display:none">
				...
				<c:if test="${not empty sessionScope.user.userId}">
					<c:if test="${sessionScope.user.userId == comment.createId}">
						<br/>
						<button class="commentCreateBtn"  type="button" onclick="toggleEditComment(${comment.commentId})">수정</button>
						<button class="commentCreateBtn"  type="button" onclick="deleteComment(${comment.commentId})">삭제</button>	
					</c:if>
					<button class="commentCreateBtn"  type="button" onclick="toggleReplyComment(${comment.commentId})">답글</button>
				</c:if>
				<!--댓글 작성 폼 -->
