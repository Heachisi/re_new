...
<div class="boardArea">
	...
	<c:if test="${not empty board.postFiles}">
		<ul>
			<c:forEach var="file" items="${board.postFiles}">
				<li>
					${file.fileName}
					<a href="/askfile/down.do?fileId=${file.fileId}">다운로드</a>
				</li>
			</c:forEach>
		</ul>
	</c:if>
	...
	function editComment(commentId){
		...
		let boardId= '${askBoard.boardId}';
		let updateId = '${sessionScope.user.userId}';
		
		ajaxRequest(
			'/askboard/comment/update.do',
			{
				commentId: commentId,
				content: content,
				boardId: boardId,
